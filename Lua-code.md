...
FA contains builtin configuration file that you can display with `fa --print-config`. Its `[Lua code]` section provides [[examples|FA 0.11 builtin Lua option definitions]] of options defined with Lua code. As of FA 0.11, `11 of 45` options are defined in Lua, including 100-line long `-mc` implementation.
...
* all built-in Lua functions plus a few functions defined by the [[builtin Lua code|FA 0.11 builtin Lua startup code]]
...
* [[other pseudofields providing read/write access to values of corresponding C++ options|FA 0.11 builtin CPP option definitions#fields]]
...
These functions are enough to implement options -lc, -ld, -md, -mm, -ms. You can learn how to deal with compression method looking at [[-mc implementation|FA 0.11 builtin Lua option definitions]].
...
Here you can find [[names of C++ options|FA 0.11 builtin CPP option definitions]] and [[names of builtin Lua options|FA 0.11 builtin Lua option definitions]].
