...
- UI/logging got the full Unicode support, displays pretty the same information as FreeArc, and implements all features of the `-di` option

- compression methods are automatically limited to the RAM size, options -lc/-ld allow to further tune the RAM usage. Default settings for compression commands is `-lc75% -ld1600m`, so archives made with default `-ld` setting may encounter problems with extraction on 32-bit systems.
...
- GenerateRandomBytes(N) returns N bytes of absolutely random data (random bytes generated by the OS are mixed using the Fortuna(SHA512) crypto-PRNG algorithm)
...
- `CompressBlockMethod/ExtractBlockMethodfunction(method,is_header,blocksize)` - process method string before compressing/extracting each solid block. `blocksize` is the solid block size. Of course, on extraction, the method transformations should keep compatibility with already compressed data, otherwise the extraction may fail.
